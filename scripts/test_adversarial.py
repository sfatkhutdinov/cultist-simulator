...
class AdversarialTester:
    ...
    def test_malformed_inputs(self):
        ...
        malformed = [
            ...
            ("Invalid context", {"action_type": ActionType.CLICK, "point": Point(100, 100)}),
        ]
        ...
        for name, action in malformed:
            context = {
                "window_bounds": self.window_bounds,
                "window_focused": True
            } if "Invalid context" not in name else {}
    ...
    def test_resource_exhaustion(self):
        ...
        action = {
            "action_type": ActionType.CLICK,
            "point": Point(999999999, 999999999)
        }
        context = {
            "window_bounds": self.window_bounds,
            "window_focused": True
        ...
        try:
            ...
            if elapsed < 0.1:  # < 100ms
                blocked = not validation.is_allowed
                self.log_attack("Huge coordinates", blocked, f"Validated in {elapsed*1000:.2f}ms")
            else:
                self.log_attack(
                    "Huge coordinates",
                    False,
    ...
    def generate_report(self):
        ...
        block_rate = (self.attacks_blocked / self.attacks_attempted * 100) if self.attacks_attempted > 0 else 0
        ...
        if self.attacks_succeeded == 0:
            ...
            for attack in self.results["attacks"]:
                if not attack["blocked"]:
                    print(f"   • {attack['attack']}: {attack.get('reason', 'No reason')}")
            passed = False
