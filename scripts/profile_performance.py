...
class PerformanceProfiler:
    ...
    def profile_vision_pipeline(self):
        ...
        self.time_function(
            "Vision Pipeline (simulated)", vision_mock, iterations=20, warmup=3
        )
    ...
    def profile_knowledge_base(self):
        ...
        self.time_function(
            "Knowledge Base Query", query_mechanic, iterations=50, warmup=5
        )

        # Profile updates
        def update_mechanic():
            kb.store_mechanic(
                "mechanic_update", {"description": "Update test", "data": {"val": 1}}
            )

        self.time_function(
            "Knowledge Base Update", update_mechanic, iterations=50, warmup=5
        )
    ...
    def profile_safety_validation(self):
        ...
        context = {"window_bounds": Rect(0, 0, 1920, 1080), "window_focused": True}

        def validate():
            validate_action(action, context)

        self.time_function("Safety Validation", validate, iterations=100, warmup=10)
    ...
    def profile_action_selection(self):
        ...
        self.time_function("Action Selection", select, iterations=20, warmup=3)
    ...
    def check_nfr_compliance(self):
        ...
        requirements = {
            "Vision Pipeline": {
                "threshold_ms": 500,
                "profile_key": "Vision Pipeline (simulated)",
            },
            "Knowledge Query": {
                "threshold_ms": 100,
                "profile_key": "Knowledge Base Query",
            },
            "Safety Validation": {
                "threshold_ms": 10,
                "profile_key": "Safety Validation",
            },
        }
