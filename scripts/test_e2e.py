...
class E2ETestRunner:
    ...
    def check_prerequisites(self):
        ...
        try:
            ...
            if window_bounds is None:
                ...
                logger.info("Available windows: (Check Activity Monitor or use window_manager.py to list)")
                return False
    ...
    def select_and_validate_action(self, game_state, window_bounds):
        ...
        try:
            ...
            if not validation.is_allowed:
                logger.warning(f"⚠️  Action blocked by safety: {validation.blocked_reason}")
    ...
    def run(self):
        ...
        # Run test cycles
        cycles_attempted = 0
        while self.actions_executed < self.max_actions and cycles_attempted < self.max_actions * 2:
            cycles_attempted += 1
    ...
    def generate_report(self):
        ...
        session_file = Path("data/sessions") / f"e2e_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Test without actually clicking"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
