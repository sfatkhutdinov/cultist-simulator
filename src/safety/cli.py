...
def main():
    ...
    bounds_parser = subparsers.add_parser(
        "check-bounds", help="Check if point is within bounds"
    )
    ...
    try:
        if args.command == "validate":
            ...
            if args.action_type == "click":
                ...
                action = Action(
                    action_type=ActionType.WAIT, parameters={}, timestamp=datetime.now()
                )
                    ...
                    json.dumps(
                        {
                            ...
                            "reason": (
                                result.reason if hasattr(result, "reason") else None
                            ),
                        }
                    )
                )
