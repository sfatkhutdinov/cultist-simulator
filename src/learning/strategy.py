...
class StrategyManager:
    ...
    def update_performance(
        self, strategy_id: str, won: bool = False, survival_time: float = 0.0
    ) -> Optional[Strategy]:
        """
        ...
        logger.debug(
            f"Updated performance for {strategy_id}: win_rate={new_win_rate:.3f}, episodes={new_episodes}"
        )
        return updated
    ...
    def evolve_strategy(
        self, base_strategy_id: str, mutation_rate: float = 0.1
    ) -> Optional[Strategy]:
        """
        ...
        logger.info(
            f"Evolved strategy {base_strategy_id} -> {new_strategy.strategy_id}"
        )
        return new_strategy

    def mutate_strategy(
        self, strategy: Strategy, mutation_rate: float = 0.1
    ) -> Strategy:
        ...
        new_strategy = Strategy(
            ...
            description=strategy.description,
            goal_priorities=(
                strategy.goal_priorities.copy() if strategy.goal_priorities else []
            ),
            exploration_rate=strategy.exploration_rate,
            risk_tolerance=strategy.risk_tolerance,
