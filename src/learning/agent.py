...
class CultistSimulatorEnv(gym.Env):
    ...
    def _state_to_observation(self, state: Optional[GameState]) -> np.ndarray:
        ...
        if state is None or self.observation_space.shape is None:
            ...
            obs_shape = (
                self.observation_space.shape if self.observation_space.shape else (550,)
            )
        ...
        grid = np.zeros(
            (self.grid_resolution[0], self.grid_resolution[1]), dtype=np.float32
        )
        if state.elements:
            for element in state.elements:
                if element.bounds:
                    ...
                    if (
                        0 <= grid_x < self.grid_resolution[0]
                        and 0 <= grid_y < self.grid_resolution[1]
                    ):
            ...
            if narrative:
                try:
                    ...
                    if analysis and "embedding" in analysis:
                        narrative_embedding = np.array(
                            analysis["embedding"], dtype=np.float32
                        )
        ...
        is_game_over = (
            state.metadata.get("is_game_over", False) if state.metadata else False
        )
    ...
    def _calculate_reward(
        ...
        is_win = (
            curr_state.metadata.get("is_win_state", False)
            if curr_state.metadata
            else False
        )
        is_game_over = (
            curr_state.metadata.get("is_game_over", False)
            if curr_state.metadata
            else False
        )
    ...
    def _get_reward_breakdown(
        self, prev_state: Optional[GameState], curr_state: Optional[GameState]
    ) -> Dict[str, float]:
