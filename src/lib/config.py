...
class SafetyConfig:
    ...
    blacklisted_keys: list = field(default_factory=lambda: [
        "cmd+q", "cmd+w", "cmd+tab", "cmd+`", "escape", "f4"
    ])
...
def _apply_env_overrides(config: AgentConfig) -> AgentConfig:
    ...
    if os.getenv("VISION_CONFIDENCE_THRESHOLD"):
        config.vision.yolo_confidence_threshold = float(os.getenv("VISION_CONFIDENCE_THRESHOLD"))
    ...
    if os.getenv("REQUIRE_WINDOW_FOCUS"):
        config.safety.require_window_focus = os.getenv("REQUIRE_WINDOW_FOCUS").lower() == "true"
