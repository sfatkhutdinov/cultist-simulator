...
class GoalExtractor:
    ...
    GOAL_PATTERNS = [
        ...
        (r'(?:you must|you need to|you should)\s+(\w+(?:\s+\w+){0,10})', GoalType.UNKNOWN, 0.9),
        (r'(?:collect|gather|obtain|find)\s+(\w+(?:\s+\w+){0,5})', GoalType.COLLECT, 0.8),
        (r'(?:survive|endure|last)\s+(?:until|through|against)?\s*(\w+(?:\s+\w+){0,5})', GoalType.SURVIVE, 0.8),
        (r'(?:reach|get to|travel to|arrive at)\s+(\w+(?:\s+\w+){0,5})', GoalType.REACH, 0.8),
        (r'(?:avoid|escape|flee from|stay away from)\s+(\w+(?:\s+\w+){0,5})', GoalType.AVOID, 0.8),
        (r'(?:unlock|open|unseal)\s+(?:the\s+)?(\w+(?:\s+\w+){0,5})', GoalType.UNLOCK, 0.7),
        (r'(?:discover|learn|uncover|reveal)\s+(\w+(?:\s+\w+){0,5})', GoalType.DISCOVER, 0.7),
        (r'(?:protect|defend|guard|save)\s+(\w+(?:\s+\w+){0,5})', GoalType.PROTECT, 0.8),
    ]
