...
def validate_image(image: np.ndarray) -> None:
    ...
    if len(image.shape) != 3:
        raise InvalidImageError(f"Image must be 3D array (H,W,C), got shape {image.shape}")
    
    if image.shape[2] != 3:
        raise InvalidImageError(f"Image must have 3 channels (RGB), got {image.shape[2]}")
    
    if image.dtype != np.uint8:
        raise InvalidImageError(f"Image must be uint8, got {image.dtype}")


def detect_elements_yolo(image: np.ndarray, confidence_threshold: float = 0.5) -> List[GameElement]:
    """
...
def detect_elements(image: np.ndarray, use_yolo: bool = True, use_templates: bool = True) -> List[GameElement]:
    """
...
def merge_overlapping_elements(elements: List[GameElement], iou_threshold: float = 0.5) -> List[GameElement]:
    """
