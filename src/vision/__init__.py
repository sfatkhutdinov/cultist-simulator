...
def detect_elements(screenshot: np.ndarray, element_types: Optional[List] = None) -> List[GameElement]:
    """
...
def extract_text_regions(screenshot: np.ndarray, regions: Optional[List[Rect]] = None) -> List[TextRegion]:
    """
...
def capture_game_state(window_name: str = "Cultist Simulator") -> GameState:
    """
...
def track_element_changes(
    ...
    prev_elements = {
        (e.element_type, e.bounds.x, e.bounds.y): e 
        for e in prev_state.elements
    }
    curr_elements = {
        (e.element_type, e.bounds.x, e.bounds.y): e 
        for e in curr_state.elements
    }
