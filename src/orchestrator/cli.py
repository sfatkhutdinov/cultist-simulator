...
def cmd_run(args):
    ...
    try:
        ...
        print(f"End Condition: {session.end_condition.value if session.end_condition else 'N/A'}")
        print(f"{'='*60}\n")
        ...
        if args.output:
            ...
            session_data = {
                ...
                'end_condition': session.end_condition.value if session.end_condition else None,
...
def cmd_train(args):
    ...
    try:
        ...
        print(f"Average Actions per Episode: {sum(s.total_actions for s in sessions) / len(sessions):.1f}")
        print(f"{'='*60}\n")
...
Examples:
  ...
  %(prog)s strategy --agent-id my_agent
    ...
    run_parser.add_argument('--window', default='Cultist Simulator', help='Game window name')
    run_parser.add_argument('--max-actions', type=int, default=1000, help='Maximum actions per episode')
    run_parser.add_argument('--max-duration', type=float, default=None, help='Maximum episode duration (seconds)')
    run_parser.add_argument('--output', '-o', help='Save session data to JSON file')
    run_parser.set_defaults(func=cmd_run)
    
    # Train command
    train_parser = subparsers.add_parser('train', help='Train agent across multiple episodes')
    train_parser.add_argument('--agent-id', default='agent_001', help='Agent identifier')
    train_parser.add_argument('--episodes', '-n', type=int, default=100, help='Number of episodes')
    train_parser.add_argument('--window', default='Cultist Simulator', help='Game window name')
    ...
    # Metrics command
    metrics_parser = subparsers.add_parser('metrics', help='Show agent performance metrics')
    metrics_parser.add_argument('--agent-id', default='agent_001', help='Agent identifier')
    ...
    strategy_parser.add_argument('--agent-id', default='agent_001', help='Agent identifier')
